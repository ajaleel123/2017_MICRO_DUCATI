...
\ee{There was a discussion about this for the other paper at the sysarch
team meeting last week before I left. They were talking about how Pascal
actually only has 32KB of L1 private cache, and its not till Volta that
we have 128KB (correct me if i'm getting this wrong and you know 
otherwise). Now in that case, they were talking about that because its
to their benefit to have smaller caches, just want to make sure we're not
being inaccurate by saying 128KB L1s for a Pascal GPU memory system. Also
is 8MB of LLC a valid-ish number for a Pascal-like memory system? We want
to be careful to not be told we're inflating our UCAT results by
considering too large of an L2} We
...
\ee{Are we showing all the workloads in all of these suites? I dont think
we are, are we? if we're doing any further selection we MUST definitely
describe how that selection is done. I'm not sure if this is currently
done.}
