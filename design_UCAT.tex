...
of the ASID in the tag array to enable at least a partial tag match.
\ee{It may be useful here to specify why we would want at least a partial
match. It would make this design choice clearer even though its not 
critical}. The bottom
...
is because recent studies show (and this study independently verifies) that a majority of LLC entries are unused after cache
...
during the course of application execution. \ee{Hmmm...can the value 10M
cycles for which sampling is done here effect what we see in the results
much? Second order importance I guess, but it does make me curious when
I read it}
...
\ee{do you want to put a pointer here that points to the following
section that discusses more implementation details? just so the reader
doesn't get worried about how short this section is?}
%<<<<<<< HEAD
