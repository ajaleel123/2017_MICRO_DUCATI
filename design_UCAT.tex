...
contains 512-1024 TLB entries~\cite{}\ee{for Aamer to fill}.
...
hit-rates and are often inefficiently used~\cite{}\ee{for Aamer to fill}, the conventional
...
insertion~\cite{}\ee{Aamer to fill}. As such, UCAT utilizes the conventional LLC space
...
Interval Prediction (DRRIP) replacement policy~\cite{}\ee{Aamer to fill}. In this
...
to the baseline system(on the y-axis) with workloads on the x-axis. The first bar in the figure shows UCAT
...
To understand the performance benefits of UCAT, Figure~\ref{fig:tlblat_UCAT}
...
experience an increase in on-die TLB size by 4x-64x.\ee{explain how you're calculating ``experiencing increased TLB size'' } On average, UCAT
...
the separate LLC, we also investigated increase in
memory accesses due to potential increase in UCAT cacheline misses
relative to the baseline. Our evaluations show that on average,
...
significant further performance improvements of up to 90\%. Additionally, workloads
...
UCAT-entries. This is evident from Figures~\ref{fig:tlblat_UCAT} and~\ref{fig:tlbsize_UCAT} where UCAT-I increases effective TLB
