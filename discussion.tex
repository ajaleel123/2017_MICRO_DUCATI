...
flush. It would be highly desirable that DUCATI does not impact the
...
for UCAT and DRAM-TLB tends to be a few tens to a few hundred nano
seconds. Thus, even in the worst case, the additional shootdown
...
in the corresponding TLB-entry match the current EPCTR of the ASID.
Both DRAM-TLB and UCAT have sufficent storage space to support a
16-bit EPCTR. As such, the TLB flush overhead can be completely
eliminated with neglible latency and hardware overhead.
