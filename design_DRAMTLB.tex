...
entries per row. \ee{This last sentence is actually inaccurate/incorrect.
I believe you're trying to say a DRAM array with a 2KB row buffer size
holds 128 DRAM-TLB entries per row. Correct? The DRAM array itself is
not 2KB.}
...
logic: \ee{make this last sentence an active sentence}
...
processor and also inserted into the DRAM-TLB. \ee{needs to be active, who does all the above, the MMU?}
...
As illustrated in Figure~\ref{fig:tlblat_DRAMTLB}, SYSMEM-TLBs reduce
