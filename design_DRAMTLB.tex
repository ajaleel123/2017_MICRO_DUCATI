...
Thus, on an LLT miss, if implemented as a stacked-TLB, the lookup fetches data at the
granularity of the stacked memory interface: 32-byte cacheline (two
TLB entries), and if implemented in sysmem, the lookup fetches data at the granularity
...
incurring additional latency or bandwidth~\cite{moin2012,loh2011}. We 
explored the design space of set-associative and direct-mapped
DRAM-TLBs. Our evaluation of set-associative and direct-mapped
...
We could further take advantage of reading multiple TLB entries with a single read 
operation. Retrieving the co-located DRAM-TLB
...
cache on a conventional page table walk. We leave the evaluation of such
...
DRAM-TLB miss, the we walk the application page table to find the
...
determine DRAM-TLB hit or miss.\ee{if we start running out of space this 
sort of thing is probably a bit redundant}
...
Figure~\ref{fig:memaccess_DRAMTLB}). \ee{A question that comes up
here is that they both seem to be reducing the number of memory 
accesses to the same extent, why is the performance of Stacked TLB so 
much better? Is there something else we should be showing/talking about?} The improved translation latency 
boosts performance of workloads like $GUPS$ and $MaxFlow$ by more than
70\% while workloads like $XSBench$, $dmr$, $LULESH$ and
...
\noindent Figure~\ref{fig:mmu_state} shows the simple modifications 
we propose to the Memory Management
Unit (MMU) state machine to support DRAM-TLBs. On an LLT miss, 
the MMU first consults
