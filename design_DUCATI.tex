...
\ee{Why does Figure 14 not have a legend?}
...
\noindent This section presents performance sensitivity of DUCATI to
LLC size. Table~\ref{table:LLC_sens} presents DUCATI performance using
4KB pages for LLC sizes ranging from 2MB to 32MB. The table shows that
on average, DUCATI improves performance across all LLC sizes studied.
In general, performance increases with growing LLC size. For example,
performance improves by 61\% with a 2MB LLC while performance improves
by 93\% with a 32MB LLC. This is because larger LLCs enable more
translation entries to be cached in the UCAT structure.
...
\ee{Why does Figure 15 not have a legend?}
